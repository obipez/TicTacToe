...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
    	...
    	for(int i = 0; i < 2; i++) {
    		...
    		if(grid[0][0] == symbols[i] && grid [1][1] == symbols[i] && grid[2][2] == symbols[i]) {
    			return message[i];
    		}

    		if(grid[0][2] == symbols[i] && grid [1][1] == symbols[i] && grid[2][0] == symbols[i]) {
    			return message[i];
    		}

    		for(int x = 0; x < 3; x++ ) {

    				if(grid[x][2] == symbols[i] && grid [x][1] == symbols[i] && grid[x][0] == symbols[i]) {
    					return message[i];
    				} 	

    				if(grid[2][x] == symbols[i] && grid [1][x] == symbols[i] && grid[0][x]== symbols[i]) {
    					return message[i];
    				}	
    			}	
    	}
